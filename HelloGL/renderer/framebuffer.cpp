	...
	:m_texture_object{width, height}, m_renderbuffer{width, height}
{
	...
	glBindFramebuffer(GL_FRAMEBUFFER, m_renderer_ID.Value());
...
void Framebuffer::AttachTextureObject(int width, int height)
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_texture_object.m_renderer_ID.Value(), 0);
}
...
void Framebuffer::AttachRenderbufferObject()
{
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_renderbuffer.m_renderer_ID.Value()); //attach to the stencil&depth attachments (can do it in one go!)
}