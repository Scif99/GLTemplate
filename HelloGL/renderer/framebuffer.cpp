...
{
	glGenFramebuffers(1, &m_renderer_ID.m_ID);
	glBindFramebuffer(GL_FRAMEBUFFER, m_renderer_ID.m_ID);
...
{
	glDeleteFramebuffers(1, &m_renderer_ID.m_ID);
}
...
void Framebuffer::AttachTextureObject(int width, int height)
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_texture_object.m_renderer_ID.m_ID, 0);
}
...
void Framebuffer::AttachRenderbufferObject()
{
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_renderbuffer.m_renderer_ID.m_ID); //attach to the stencil&depth attachments (can do it in one go!)
}